...
public class TradeWebSocketHandler extends TextWebSocketHandler {
        ...
        if ("subscribe".equals(action)) {
            ...
            if (!tasks.containsKey(session)) {
                ScheduledFuture<?> future = scheduler.scheduleAtFixedRate(
                        () -> sendTrades(session), 500, 500, TimeUnit.MILLISECONDS);
    ...
    private void sendTrades(WebSocketSession session) {
        if (!session.isOpen()) {
            ScheduledFuture<?> future = tasks.remove(session);
            if (future != null) future.cancel(false);
            return;
        ...
        for (String symbol : symbols) {
            try {
                ObjectNode trade = marketDataService.generateTradeTick(symbol);
                session.sendMessage(new TextMessage(mapper.writeValueAsString(trade)));
            } catch (IOException e) {
                log.debug("Failed to send trade to session {}", session.getId());
            }
        }
    }
}
